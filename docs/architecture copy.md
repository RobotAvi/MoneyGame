# Архитектура (устаревший файл)

Актуальная версия: `ARCHITECTURE_CURRENT.md`

Целевая архитектура: `ARCHITECTURE_GOALS.md`

## Обзор архитектуры

Проект "Финансовый успех" использует классические Android-компоненты (Activities, RecyclerView, ViewBinding) и четко отделенную бизнес-логику (GameManager, модели в `models/`). MVVM/ViewModel/Compose и DI отмечены как планы на будущее в документации, но не применяются в текущем коде. Архитектура обеспечивает масштабируемость, тестируемость и поддерживаемость кода.
...
- **ViewModels**: (планируется) Управление состоянием UI
